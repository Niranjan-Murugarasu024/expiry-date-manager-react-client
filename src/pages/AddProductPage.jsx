...
import { createProduct, lookupCatalogByUpc } from '../utils/api';

export default function AddProductPage({ currentUser, onNavigate }) {
    ...
    setTimeout(async () => {
      try {
        ...
        await html5QrCode.start(
          ...
          (errorMessage) => {
            // Scanning in progress... silent ignore individual frame errors
          }
        );
      } catch (err) {
  ...
  const handleBarcodeScanned = async (code) => {
    ...
    // Auto-trigger master catalog lookup
  ...
  const performUpcLookup = async (upcCode) => {
    if (!upcCode || !upcCode.trim()) {
      setLookupMessage({ type: 'error', text: 'Please enter a UPC barcode code.' });
      return;
    }
    ...
    try {
      ...
      if (res.found && res.catalogItem) {
        ...
        setLookupMessage({
          type: 'info',
          text: 'UPC barcode not found in Master Catalog. Enter product details manually below to register it!',
        });
      }
    } catch (err) {
      setLookupMessage({
        type: 'error',
        text: 'Could not lookup UPC in catalog. You can still enter details manually.',
      });
    } finally {
      setLookingUp(false);
    }
  ...
  // Preset Expiry Date helper (+N days)
      ...
      <div className="max-w-3xl mx-auto space-y-8">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-stone-400">Point your device camera at product barcode (UPC-A, EAN-13, etc.)</p>
              </div>
            </div>
          ...
          {isScanning && (
            <div className="space-y-3 pt-2">
              ...
              <p className="text-center text-xs text-amber-400/90 font-medium">
                Hold barcode steady. The scanner will detect the UPC code automatically.
              </p>
            </div>
          )}
        ...
        <div className="bg-stone-900/90 border border-amber-900/40 rounded-3xl p-6 sm:p-8 shadow-2xl space-y-6">
          <div className="border-b border-stone-800 pb-3">
            <h2 className="text-lg font-bold text-white">Product Inventory Form</h2>
            <p className="text-xs text-stone-400">Enter or verify product information to save to your pantry list.</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div>
              <label className="block text-xs font-semibold text-stone-300 uppercase tracking-wider mb-1.5">
                UPC Barcode Code (Manual or Camera)
              </label>
            ...
            {/* Expiry Date Picker & Quick Presets */}
            <div>
